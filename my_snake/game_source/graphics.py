...
class Point(deque):
    ...
    def __add__(self, other): return Point(
        [v + w for v, w in zip(self, other)])

    def __radd__(self, other): return Point(
        [w + v for v, w in zip(self, other)])

    def __sub__(self, other): return Point(
        [v - w for v, w in zip(self, other)])

    def __rsub__(self, other): return Point(
        [w - v for v, w in zip(self, other)])
